...
| `correlacoes/` | Correlações · Gestão em Movimento | padrão | claro | Sheets:DRE, Sheets:Dispersão, sb:elite_snapshot (gerot-base), sb:fca, sb:disp_resumo, sb:custo_vigencia_mv, sb:ginfo_snapshot, sb:indisponibilidade, sb:ce_scores_mensais | 2 | scatter+line, bar | dre + matriz | Excel/PNG · PDF · ordenação | 202609240100 |
...
| `estatistica.js` (~230 l) | Motor do painel de Correlações, sem DOM: `Estat.{pearson, spearman, reta, faixaReta, ols, bh, pT, tCrit, defasagens, descreve, forca, N_MIN}`. No node é `module.exports` (o `scripts/estatistica-teste.mjs` confere cada função contra valores de referência do simple-statistics e do jStat). | correlacoes |
