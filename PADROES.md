...
| `correlacoes/` | Correlações · Gestão em Movimento | padrão | claro | Sheets:DRE, Sheets:Dispersão, sb:elite_snapshot (gerot-base), sb:fca, sb:disp_resumo, sb:custo_vigencia_mv, sb:ginfo_snapshot, sb:indisponibilidade, sb:ce_scores_mensais (via `correlacoes-dados.js`) | 2 | scatter+line, bar | dre + matriz | Excel/PNG · PDF · ordenação | 202609240400 |
| `correlacoes-operacao/` | Correlações da Operação · Gestão em Movimento | padrão | claro | as mesmas do `correlacoes/` (via `correlacoes-dados.js`); unidade de `fca_profiles.unidade` | 1 (Vigência; admin ganha Unidade) | bar, scatter+line | dre | Excel/PNG · PDF · ordenação | 202609240400 |
...
| `correlacoes-dados.js` (~340 l) | A tabela-fato das Correlações, uma só para os dois painéis: `CorrDados.{VARS, FMT, varDe, fmtVar, vigKey, vigRot, vigIdx, CIDADE2COD, comTier, uniCodCE, carregar(sb)}`. `carregar` lê DRE Frota (por rótulo, custo com o sinal invertido), Dispersão de km, Gerot, fca, disp_resumo, custo_vigencia_mv (+ativos e indisponibilidade) e ce_scores_mensais com `Promise.allSettled`, e devolve `{FATO:{uni,placa,mot},FONTES}` — linhas `{ent,uni,cod,vig,v:{}}`, `cod` = código da unidade do portal em todo grão. | correlacoes, correlacoes-operacao |
