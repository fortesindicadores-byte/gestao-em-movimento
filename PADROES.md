...
| `correlacoes/` | Correlações · Gestão em Movimento | padrão | claro | Sheets:DRE (Frota + Receita Líquida), Sheets:Dispersão, sb:elite_snapshot (gerot-base), sb:disp_resumo, sb:custo_vigencia_mv, sb:ginfo_snapshot, sb:indisponibilidade, sb:ce_scores_mensais (via `correlacoes-dados.js`) | 2 | scatter+line, bar, quadrantes | dre + matriz | Excel/PNG · PDF · ordenação | 202609240500 |
| `correlacoes-operacao/` | Correlações da Operação · Gestão em Movimento | padrão | claro | as mesmas do `correlacoes/` (via `correlacoes-dados.js`); unidade de `fca_profiles.unidade` | 1 (Vigência; admin ganha Unidade) | bar, scatter+line, quadrantes | dre | Excel/PNG · PDF · ordenação | 202609240500 |
...
|---|---|---|
| Administração | Gerenciar Acessos (tela interna) · Acessos `acessos/` · Conferência de Locação `conferencia-locacao/` · Bases do Sheets (janela interna) · Saúde do Ecossistema `saude/` · Check de Metas `check-metas/` · Correlações `correlacoes/` | cluster só admin |
| Planejamento Estratégico | Estratégia e Governança `governanca/` · Papéis e Responsabilidades `papeis-responsabilidades/` | |
| Visão Geral | Scorecard `scorecard/` · Diagnóstico `diagnostico/` · Resumo Executivo `resumo-executivo/` · Ativos `ativos/` · Gestão à Vista `gestao-a-vista/` · Correlações da Operação `correlacoes-operacao/` | |
...
|---|---|---|
| `gviz-cache.js` | No `<head>`. Intercepta `fetch` **e JSONP** ao `docs.google.com/.../gviz/tq` (e o `/export?format=csv`) e responde do **banco** (tabelas `sh_<slug>`, payload gviz reconstruído) → `gviz_snapshot` (reserva, até 12 h) → Google (só sem tabela; contado em `GvizCache.google`). Lê a tabela inteira em **página adaptativa**: pede 10.000 e o passo vira o que o servidor devolveu (Max rows da API). Dentro do Check de Metas (`window.__gvizMem` na página-mãe) guarda abas e GETs ao `/rest/v1/` e o iframe repetido não volta ao banco. Chave: `"<sheet_id>|s=<aba>|g=<gid>|q=<tq>|h=<headers>"`, byte a byte igual à do robô; `APELIDOS` espelha o `sheets-bases.mjs`. `window.GvizCache.{hits,misses,banco,snapshot,google,fontes}`. | todos os painéis que leem Sheets (financeiro-pessoal e controle-ficticio ficam fora de propósito) |
...
| `estatistica.js` (~230 l) | Motor do painel de Correlações, sem DOM: `Estat.{pearson, spearman, reta, faixaReta, ols, bh, pT, tCrit, defasagens, descreve, forca, N_MIN}`. No node é `module.exports` (o `scripts/estatistica-teste.mjs` confere cada função contra valores de referência do simple-statistics e do jStat). | correlacoes, correlacoes-operacao |
| `correlacoes-dados.js` (~360 l) | A tabela-fato das Correlações, uma só para os dois painéis: `CorrDados.{VARS, FMT, varDe, fmtVar, vigKey, vigRot, vigIdx, CIDADE2COD, comTier, uniCodCE, carregar(sb), porEntidade(rows), trivial(vx,vy)}`. `carregar` lê DRE Frota (por rótulo, custo com o sinal invertido), DRE Receita Líquida, Dispersão de km, Gerot, disp_resumo, custo_vigencia_mv (+ativos e indisponibilidade) e ce_scores_mensais com `Promise.allSettled`, e devolve `{FATO:{uni,placa,mot},FONTES}` — linhas `{ent,uni,cod,vig,v:{}}`, `cod` = código da unidade do portal em todo grão. Cada variável leva `dir` (up/down = lado bom) e `b` (bases: par com base em comum é trivial e fica fora da matriz/achados/regressão). FCA não entra. | correlacoes, correlacoes-operacao |
| `correlacoes-quadrante.js` (~90 l) | A visão Quadrantes dos dois painéis: `CorrQuad.monta(ents,vx,vy,{ref})` (um ponto por entidade, referência = média ou mediana, quadrante de cada ponto, melhor/pior pela `dir`) e `CorrQuad.desenha(canvas,q,vx,vy,{Chart,estilo,fmtVar,destaque,rotulo,plugDL})` (scatter + plugin `quad` que tinge o melhor/pior, traça as referências e rotula os cantos). | correlacoes, correlacoes-operacao |
...
PWA (`manifest.json`, `sw.js` `driverpro-v20`, Inter, tokens próprios `--bg:#0F1A20 --acc:#F22C05`; tema claro por classe). Fala com o banco **só por RPC** (`ce_app_login`, `ce_app_dados`, `ce_app_ping`…). Regras: unidades no programa (`ce_app_regras.unidades`, hoje só `EMP PIRAI`); critérios por unidade e **grupo** (`ce_app_unidade_cfg`: Empurrada top 15/1.000 km, Lata top 5/500 km, um ranking por grupo PIRAI); **ranking entre quem disputa** (`pos` só para `disputa`; `posicao_geral` vai junto); número grande = **carteira + pódio**, barra = só carteira, linha "Prêmio do pódio" no card; pilares *Uso da faixa verde (até 1.700 RPM)* · *Motor ligado sem rodar* · *Acelerações bruscas*; freada não pontua. Domínio `driverpro.onespot.com.br` pelo repositório `fortesindicadores-byte/driverpro` (sync a cada 5 min, com CNAME; disparar na mão ao publicar). O app **se atualiza sozinho**: `cache:'no-store'` na navegação e no HTML/JSON, `updateViaCache:'none'` no registro, `update()` ao abrir e ao voltar, recarga única no `controllerchange`. Log de uso em `ce_app_log`; painel Condução Econômica tem visões Acessos e Ranking de Acessos, Carteira em R$, Km/L Gerencial e Ranking Km/L (litros do `FuelUsed`; km/L = Σkm ÷ Σlitros **só das viagens com litros**, descartando >12 km/L).
