...
**BALANÇO DE MASSA — recomposição do km remunerado das EMPURRADAS (Renan, 08/09/2026):** aba **`Balanço de Massa`** (`Unidade | Vigência | Valor`) no workbook Base Dispersão de km (`GV_ID`). São viagens que não emitiram CTE e foram cobradas depois; o valor **volta ao km remunerado** da Dispersão de km nas três empurradas (CUIABA EMPURRADA → `EMPURRADA|CBA`, PIRAI EMPURRADA → `EMPURRADA|PIR`, MACACU EMPURRADA → `EMPURRADA|MCC`; a vigência da aba vem como `mmm/aa` ou `MM/AAAA`). Regra: **km recomposto = Valor ÷ R$/km dos variáveis**, com R$/km variável = Σ remunerado das cinco contas **Manutenção de Veículos e Equip. · Manutenção de Carrocerias · Pneus Novos · Recapagens e Outros Serviços · Combustíveis** (aba `Frota` do DRE, chave vigência|projeto|unidade) ÷ **km remunerado ORIGINAL** da chave — a taxa é calculada ANTES de somar o recomposto, senão vira circular. O km recomposto é rateado entre as linhas da chave proporcionalmente ao remunerado original (`rem0`, `recomp`, `bmValor`, `bmTaxa` nas linhas do `painel-km`); o R$/km do impacto (3 pacotes) continua sobre o km original. **Só o Painel KM e a Árvore de Combustível aplicam** — os outros 8 leitores da `Dispersão de km` (Seara, Gerot, Termômetro…) seguem com o km bruto. No Painel KM aparece no hero (`Balanço de massa: +X km · R$ Y`); na Árvore a **caixinha "Recomposição · Balanço de Massa"** (Valor R$ · R$/km var. · Km no Rem) desce do card Km Rodado Remunerado com uma seta, para mostrar quanto do remunerado veio da recomposição (o remunerado já a inclui). Auditoria: workflow **Balanco Massa Check** (`scripts/balanco-massa-check.mjs`), que imprime antes × depois por unidade e por mês — foi dele que saiu `docs/balanco-massa-comparativo.pdf` (jan→jul/2026: R$ 3,93 mi → +896 mil km, 24,7% do km remunerado; em Piraí é 27% e o Δ km inverte de perda para ganho). A aba está nos `ALVOS` do gviz-robot. Chave sem km remunerado ou sem custo variável na Frota é ignorada e listada no console (`bmSem`).
