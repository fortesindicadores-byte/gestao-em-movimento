...
**GRL 95% NO FROTA DE ELITE × −7,7% NO PAINEL KM/L (Renan, 24/09/2026):** mesma aba, mesmo realizado (Σkm ÷ Σlitros), remunerados diferentes. O painel (`aggRows`, modo "medio", OFICIAL) faz média simples do `KM/l Rem Médio` POR PROJETO e pondera os projetos pelo km rodado; o `combUm` do gerot-base faz média simples de TODAS as linhas placa-mês da filial. O painel pesa km, o Elite pesa placa — onde um projeto tem muitas placas de pouco km (vans, apoio) os dois se afastam. Auditoria: workflow **KmL Elite vs Painel** (`scripts/kml-elite-vs-painel.mjs`, inputs `vig`/`uni`), que imprime as duas contas e a abertura por projeto. Decisão de qual régua o Elite usa é do Renan.
