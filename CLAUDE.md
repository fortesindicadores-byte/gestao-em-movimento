...
- **BLITZ DE SEGURANÇA** (menu **SEGURANÇA → BLITZ DE SEGURANÇA**, 09/09/2026): exportar a tabela **"ADERÊNCIA MENSAL POR PLACA"** (Placa | Aderência | Nunca Realizado | Não Realizado | Realizado Fora Prazo | Realizado Dentro Prazo | No Prazo | Tempo Médio) → chave `blitz-seguranca`. O robô a acha pela coluna **"Nunca Realizado"**, que a tabela por Regional do topo da página NÃO tem — mirar por "Placa" ou "Aderência" pegaria a de cima. **O export não traz filial nenhuma**: a unidade de cada placa sai do join com a base `ativos`, igual às Preventivas. As cinco contagens são as MESMAS da Conformidade, então a régua é a mesma: **Aderência = (Realizado Dentro Prazo + No Prazo) ÷ soma das cinco** (conferido contra a tela: 0/0/1/0/1 → 50%, 0/1/1/1/2 → 60%). Por serem contagens, a unidade sai do **pool das placas**, nunca da média das aderências. A aba nasceu **`opcional:true`** — falha dela não derruba as outras sete do run diário. **PERÍODO EM ABERTO (validar com o Renan):** a 1ª coleta (09/09/2026) trouxe **746 placas** e o rodapé do export diz que o visual estava com `Aderência OK não está em branco | ds_empresa é CONLOG` — **nenhum filtro de mês**, e a tabela **não tem coluna de competência**. Ou seja, o número é o que a página mostra por padrão; se for preciso fixar o mês (como no `checklist-031120`), falta descobrir o slicer. Leitor: `DATA.blitz` no `farol-core.js` e a visão **Blitz de Segurança** do Gestão à Vista (`renderBlitz`), que só aparece depois da primeira coleta (`temDado`).
