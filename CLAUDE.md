...
## O BANCO CAIU SOB LEITURA — Nano → Micro, memória do deck, página adaptativa, hub sem espera (24/09/2026)

Renan gerava o PPT do Check de Metas (*"Porra, que demora isso"*: 19 slides em 822 s, slides de FCA vazios), depois o Check de Metas ficou em "Carregando…" e o **hub abriu em branco**. Medido pelo Actions: `fca` respondendo **HTTP 522** da Cloudflare em 19 s; o dashboard mostrava Status "Checking…", Postgres com erros na barra das 21h e **RAM 68% com o banco parado**. Status geral do Supabase verde — era o **projeto**, no compute **Nano** (0,5 GB, o tamanho do plano Free; o projeto nasceu no Free e o plano Pro veio depois). Já tinha acontecido em 21/08 (aviso de Disk IO Budget).

- **O que pesa**: cada abertura de painel lê tabelas `sh_*` INTEIRAS, em páginas de 1.000 disparadas em paralelo (Visão Financeira ≈ 36 leituras; Painel de Metas 50 mil linhas/59 leituras; DRE Frota 17.810 linhas é lida por 8 painéis). O Check de Metas abria um iframe novo por slide e cada iframe relia tudo — 46 slides ≈ mil leituras de tabela grande em 20 min. Num Nano nada fica em cache: cada leitura vai ao disco e o orçamento de IO acaba. Inventário de peso por painel: `docs/gviz-inventario.json` × `linhas` do log do Sheets Robot (script de bolso na sessão; refazer se precisar).
- **Renan subiu para MICRO** (1 GB; o Pro inclui US$ 10/mês de compute e o Micro custa isso — custo zero). Sinal para subir de novo: RAM acima de 85% no dia a dia ou o Disk IO budget zerando; o Small custa US$ 5 a mais.
- **`assets/gviz-cache.js` lê em PÁGINA ADAPTATIVA**: pede 10.000 (`PAG_MAX`), o servidor devolve o teto dele ("Max rows" da API, 1.000 por padrão) e o passo das leituras seguintes é o que voltou. **Subir o Max rows para 10.000 em Project Settings → API** corta a Visão Financeira de 36 leituras para 4 sem mexer em código — ainda não feito.
- **Memória da geração no Check de Metas**: a página-mãe expõe `window.__gvizMem = {}` e o shim dos iframes (mesma origem) guarda ali as abas reconstruídas (por chave, só o CORPO em texto — o obj vai por referência ao painel via JSONP e é reconstruído a cada hit) e os **GETs ao `/rest/v1/`** (por URL + Range/Prefer/Accept, com status e Content-Range). O painel repetido não volta ao banco; o log final diz `Banco: X leitura(s) · Y servida(s) da memória`. Sem a mãe, `MEM` é null e nada muda no portal. A memória nasce vazia a cada geração (deck do mesmo instante) e é zerada no `finally`.
- **Hub não espera o banco**: os clusters são texto fixo no HTML e o `showHub` esperava `fca_profiles` SEM teto antes de desenhá-los (duas vezes: rotear gestor e saber se é admin). Agora: perfil em cache (`gem_perfil_<uid>`), tela desenhada com o último papel conhecido (ou só pelo e-mail do admin), leitura com teto de 5 s (`lePerfil`), gestor roteado pelo cache quando o banco não responde, e a faixa `#hub-aviso` ("Banco de dados sem resposta desde HH:MM") em vez de silêncio. O `maybeRouteByRole` deixou de existir.
- **O que o hub NÃO resolve**: os painéis que vivem só no banco (FCA, Gestão à Vista, Carta, Indisponibilidade, Check de Metas) ficam parados até ele voltar; os de planilha caem para o snapshot, que é o mesmo banco, e depois para o Google.
- **Validação**: `scripts/gviz-memoria-teste.mjs` (Chromium, 29 checagens, `SBJS` = UMD do supabase-js baixado do npm) — página adaptativa com teto 1.000 e 10.000, 2º e 3º iframe com ZERO leituras, memória não vaza fora da mãe, hub com banco mudo (admin, cache, sem cache), banco respondendo, gestor roteado pelo banco e pelo cache. **Rodado nos dois lados**: com o shim e o hub antigos falham 4 e 9 checagens. Três armadilhas foram do DUBLÊ: o supabase-js `.single()` pede `Accept: application/vnd.pgrst.object+json` e quer UM objeto (lista → PGRST116); a rota `fca` casava `fca_profiles` antes dela; e o Content-Range só chega ao navegador com `Access-Control-Expose-Headers`.
- **Obsoleto pesando** (medido no inventário, não mexido): **`seara_ctes` (57.679 linhas, a maior tabela) não é lida por painel nenhum** desde 16/09 e segue sendo carregada e fotografada de hora em hora; `disp_disponibilidade` (19.559) e `disp_indisponibilidade` (13.628) só servem à reserva do Gestão à Vista, que é o próprio banco; a foto da `Árvore Comb.`; e as cópias de teste publicadas (`carta-custos-teste`, `planner-teste`, `planner-teste-2`, `hub-classico`, `visao-financeira-cheia`, `fca-migracao`, `disponibilidade-migracao`). Decisão do Renan.
- **CÓPIAS DE TESTE APAGADAS (Renan, 24/09/2026: "Cópias exclua todas")**: `carta-custos-teste`, `planner-teste`, `planner-teste-2` (+ `assets/planner-teste-shim.js`, que só elas usavam), `hub-classico`, `visao-financeira-cheia`, `fca-migracao` e `disponibilidade-migracao` saíram do repositório. Nada linkava para elas (conferido por grep; o `planner-corporativo` só as citava num comentário). Voltam com `git show <commit>^:<pasta>/index.html`. A `app-motorista-5` (cópia em Montserrat) NÃO estava na lista e ficou.
- **TRÊS BASES MORTAS SAÍRAM DOS ROBÔS E DO BANCO (Renan, 24/09/2026: "Pode fazer")**: `seara_ctes` (57.679 linhas, a maior tabela — cópia crua da aba Base CTEs, sem leitor desde que o km remunerado da Seara passou para a aba `Remunerado` em 16/09; a contagem de viagens sai da agregada `seara_ctes_viagens`, que FICA), `disp_disponibilidade` e `disp_indisponibilidade` (abas congeladas desde o desligamento do Apps Script em 19/09; a "reserva" do Gestão à Vista morava no mesmo banco e nunca serviria). Saíram do `sheets-bases.mjs` (42 bases), dos `ALVOS` do `gviz-robot` e o SQL de `drop table` + limpeza de `sh_base`/`gviz_snapshot` foi colado no chat (`scripts/bases-mortas.sql`). **A reserva da planilha saiu do `farol-core.js`** (`loadDispSheet`/`loadInd`/`DISP2CT`/`DISP_SHEET_ID` apagados): banco mudo vira `DATA.fonte.disp={src:'erro',motivo}` e a legenda "Banco de dados sem resposta" — também no estado vazio do `renderDisp` —, sem nenhum pedido ao Google (o teste conta os pedidos ao docs.google e exige zero). O workflow **Disp Fonte Check** (planilha × banco) foi apagado junto: comparar com uma aba congelada não mede nada. `disp-fonte-teste.mjs`: 25 ok; a fixture tinha `data_parada` FIXA em 18/09 (escrita em 19/09) e envelhecia — virou "ontem" calculado. O `bases-manuais.sql` (gerado) ainda cita as três tabelas; regerar pelo Sheets DDL na próxima vez que rodar.
- **Achado do log do dia**: a aba `Dispersão de km` estava FILTRADA (26 linhas contra 998), o porteiro recusou e o banco manteve a última boa. O **Saude Robot** falhava desde 23/09 à tarde com `PGRST102: All object keys must match` ao gravar `saude_base`: o PostgREST recusa insert em massa com objetos de chaves diferentes, e as linhas do Elite e dos aplicativos não levavam `impressao` (as do Sheets/gviz/Ginfo levam). Corrigido em 24/09 ("Faça 1 e 3"): `mesmasChaves()` no `grava()` completa todo lote com a união das chaves (faltante = null). Teste `scripts/saude-grava-teste.mjs` roda a função do próprio arquivo (5 ok).
...
- **CUSTO DA DRE ENTRA COM O SINAL INVERTIDO (bug real, 23/09/2026, primeira leitura com dado de verdade):** a regressão de Custo total ~ Km realizado saiu com β = −0,97, e o Renan perguntou *"aqui está dizendo que quando km real sobe custo desce?"*. Estava — porque a aba Frota guarda a despesa como número **negativo** (o Painel KM inverte com `-(valor)`, o fca-preenchimento tem o comentário "Custos na DRE são negativos") e o painel lia o cru: "gastou mais" virava "mais negativo" e toda correlação com custo saía de cabeça para baixo. Agora o `loadDRE` nega rem e real (crédito como o ICMS abate, como na Visão Financeira), e a fixture do teste passou a vir negativa como a aba: com o código antigo o teste dá r = −0,98 entre custo e km; com o novo, r > 0,9. Lição para qualquer variável nova de dinheiro: **conferir o sinal da fonte antes de correlacionar**.
...
### A cópia para a operação: `/correlacoes-operacao/` (Renan, 23/09/2026: "uma cópia mais amigável para a operação")

Ele viu o painel admin e disse *"ficou confuso"*; a explicação em português foi o que fez sentido, e virou a tela. Escolhas dele: formato **"os dois"** (perguntas prontas na entrada + Explorar traduzido) e recorte **"só a própria unidade, com a rede como referência"**. Card **Correlações da Operação** no cluster **Visão Geral**, para todo logado.

- **A tabela-fato saiu do painel admin para `assets/correlacoes-dados.js`** e os dois painéis a leem — refazer a leitura na cópia daria dois números para o mesmo indicador. O motor continua o `estatistica.js`. O painel admin não mudou de conta: o teste dele seguiu com as mesmas checagens depois da extração.
- **A unidade vem de `fca_profiles.unidade`** (lista por vírgula → chips no topo, como no FCA); **admin** escolhe qualquer uma no filtro Unidade (PIR por padrão); usuário **sem unidade** cai no gate "Sem unidade no perfil" antes de ler fonte nenhuma. **Nada de conta com uma unidade só**: 8 meses são 8 pontos, abaixo do mínimo — a conta é sempre feita com **todas as unidades** do recorte de vigências, e a do perfil aparece **em laranja** contra a rede em cinza (dispersão) ou em azul (barras por mês). O subtítulo diz isso (`PIR contra a rede · 105 unidade-mês`).
- **Resumo Gerencial = 6 perguntas prontas**, cada card com pergunta, o número da unidade, a rede como referência ("8% acima da rede", vermelho ou verde conforme o sentido bom; abaixo de 0,5% vira "na média da rede"), um gráfico, a leitura em português e as tags de rodapé: *Quanto custa cada km em combustível?* (Σcomb ÷ Σkm, barras unidade × rede) · *Quanto vale ganhar 0,1 km/L?* (OLS comb_real ~ km_real + km/L, o coeficiente × 0,1 vira R$/mês — é a conta que ele pediu no chat) · *Rodar acima do remunerado estoura o combustível?* · *Preventiva atrasada vira manutenção no mês seguinte?* (defasagem) · *Quando o conserto demora, a disponibilidade cai?* · *Caminhão mais velho custa mais por km no contrato?* (grão placa).
- **A tradução é regra, não estilo:** nada de r, β, p ou R² na tela. `forcaTxt(r)` → "ligação forte/moderada/fraca/quase nenhuma"; `coincTxt(p)` → "muito difícil ser coincidência / dificilmente / pode ser coincidência"; a regressão vira colunas **Peso · Efeito · Confiável?**; no card do Km/L o β padronizado é rotulado como **efeito** (grande/médio/pequeno), não como força de ligação — senão "quase nenhuma ligação" briga com "relação firme" no mesmo card. O teste afirma que nenhum card fala em Pearson, β, p-valor ou R².
- **Explorar** são as três visões do admin com os mesmos seletores (Dois indicadores · O que explica · O que vem antes) e, na dispersão, uma tabela com **os meses da própria unidade contra a tendência** (acima = verde).
- **Validação** no mesmo `scripts/correlacoes-teste.mjs` (cenários 5–8, 83 checagens no total): perfil com uma unidade, com duas (chips), admin (seletor), sem unidade (gate), fonte com erro (aviso e as 6 perguntas seguem). O dublê do `fca_profiles` ganhou o campo `unidade`. **Um bug de gramática só apareceu no teste**: "tende a sobe".

### Quadrantes · rem × real não é correlação · FCA fora (Renan, 23/09/2026)

Três pedidos dele na primeira leitura do painel com dado de verdade, publicados juntos:

- **"Um gráfico com quadrantes e duas variáveis, a exemplo do CTO/ROB… e onde é melhor estar"** → visão **Quadrantes** nos dois painéis (`assets/correlacoes-quadrante.js`): X e Y à escolha entre todas as variáveis, **um ponto por entidade** (média dos meses do recorte, `CorrDados.porEntidade`), referência = **média ou mediana** da rede, quatro quadrantes rotulados `+ X · − Y`, e o **melhor/pior pela direção de cada variável** (`dir:'up'|'down'` no catálogo: custo, desvio, R$/km, dispersão, MTTR, OS vencidas e indisponíveis são "menor é melhor"; aderências, km/L, MTBF, disponibilidade e receita são "maior é melhor"). Variável **sem direção** (km rodado, viagens, litros…) mostra os quadrantes sem apontar o melhor, e a leitura diz por quê. No painel da operação a própria unidade vai em laranja com a leitura "Sua unidade está em …". **Receita Líquida** entrou como variável (`receita` e `rec_km` = receita ÷ km real), lida da aba `Receita Líquida` do DRE pelos mesmos rótulos da Frota.
- **"Não faz sentido correlacionar remunerado com realizado… Que idiotice"** → cada variável leva `b` (as **bases** de que sai: `custo`, `comb`, `km`, `disp`…), e um par com base em comum é **trivial**: custo total × combustível (parte dele), desvio × o próprio custo, R$/km × km, rem × real, nota × um pilar da nota. Trivial vira `—` na matriz (com a explicação na dica), não entra nos achados nem no BH, fica **desabilitado** na lista de X da regressão quando a base bate com o Y, e vira aviso na dispersão e na defasagem. `Custo remunerado` e `Km remunerado` **deixaram de ser variáveis** — continuam alimentando desvio e dispersão. O **par padrão dos seletores nunca é trivial**: o Y pula para a primeira variável de outra base (sem isso, a tela abria com receita × R$/km de receita).
- **"FCA não deve estar nisso"** → a fonte `fca` e as cinco variáveis saíram dos dois painéis. A tabela continua no dublê do teste só para provar que ninguém a pede.
- **Validação**: `correlacoes-teste.mjs` com **101 checagens** — quadrante no admin (13 pontos, melhor = −Disp +Km/L, quatro cards somando 13, quadrante de PIR conferido contra a referência, mediana, variável sem direção) e na operação (rede em cinza 12 + PIR em laranja, leitura da própria unidade, a linha dela primeiro na tabela); pares triviais com `—` e fora dos achados; regressão com Y = custo desabilitando Combustíveis/R$/km/desvio; Receita entrando com `rec_km` = 3,2 por construção.
...
**A ÁRVORE DO MCC T2 SAIU COM TODAS AS UNIDADES (Renan, 24/09/2026: "Imagem 1 é a árvore MCC T2 do ppt gerado, e imagem 2 filtrando o projeto rota. Nada a ver os números").** 569 mil km no slide contra 63 mil na tela. A causa é o mesmo tipo de desencontro de dialeto das vigências: **o filtro Unidade da Árvore de Combustível lista o SUFIXO do nível 3** (`ROTA - MCC` → `MCC`), e a tabela `fca` guarda `MCC T2`. O `__cm.sel` não achava, **limpava o filtro** (é o desenho: seleção presa é pior) e a árvore saía com o ROTA de todas as unidades — com o aviso só no Roteiro, que ninguém lê quando o slide parece bonito. Só as unidades com tier (CBA T1/T1 WH/T2, MCC T1/T2) sofriam; CGR, PIR, GRL casavam por igualdade. A unidade agora vai com a alternativa **sem o tier** (`semTier`: `MCC T2|MCC`), e o tier já está garantido pelo projeto (`EMPURRADA` = T1, `APOIO` = T1 WH, o resto = T2).
- **O teste não pegou porque o dublê listava `CBA T1` em TODOS os painéis** — validava um mundo em que a Árvore conhece o tier. O dublê da Árvore (dialeto `mm`) passou a listar `CBA`, e a fixture ganhou um Combustíveis do `CBA T1`; rodado nos dois lados: com o código antigo o filtro sai `[]`, com o novo sai `["CBA"]` (200 checagens).
- **"41454 não 40706" NÃO é bug, é definição:** o R$ do subtítulo vem do FCA, que é o **pacote** Combustíveis do fca-preenchimento (conta Combustíveis **+ Arla**); a Árvore mostra **só a conta Combustíveis** (`COMB`, "conta é conta", sem Arla). Os R$ 748 de diferença são o Arla. Cheguei a pôr no subtítulo da árvore "desvio do pacote no FCA: R$ X · a árvore mostra só a conta"; ele cortou na hora (*"Tire esse desvio ali. Confunde. Deixe apenas o árvore de combustível e a vigência"*). **O slide da árvore leva só `Árvore de Combustível · ago/2026`**; o R$ do desvio fica no slide do FCA, que é de onde ele sai.

**O DECK LEVOU 16.703 s (4h38) PARA 46 SLIDES e o log não dizia onde (Renan, 24/09/2026: "Demorando absurdamente para gerar. Normal isso?").** Descartado por medição: não era o banco (45 leituras, 195 da memória) nem a espera por painel (tem teto de 60 + 70 s e estourar marcaria "atenção" — nenhum slide marcou). Sobram a foto/leitura de cada painel e a MONTAGEM (um html2canvas 2× por página, depois que a barra já mostra 46/46), mais a hipótese de a aba ter ficado em segundo plano. Em vez de chutar, o gerador ganhou um **cronômetro** (`CRONO`/`relataCrono`): o log final diz o tempo de abrir painéis · esperar os dados · ler/fotografar · montar o arquivo (com nº de páginas), quanto tempo a aba ficou escondida (`visibilitychange`) e os três slides mais lentos. Sai também quando a geração dá erro. Próxima geração lenta → ler essas linhas antes de mexer em qualquer coisa.

**FUNDO DO SLIDE = `#F0F0F0` (Renan, 24/09/2026: "não é esse cinza do fundo o correto. É o da imagem. Esse cinza fica cinza demais").** O slide e a captura dos painéis usavam `#E1E2E5` (a página do portal no claro), mais escuro e puxado para o azul; o certo é o cinza claro da página dos painéis no tema claro, medido no pixel do print dele. Token `TK.fundo` no `check-metas/slides.js` (a captura herda dele). **A capa diz só "GESTÃO EM MOVIMENTO"** — o "· FORTES INDICADORES" saiu no mesmo dia, a pedido dele.

**VAN É UM PROJETO SÓ NA ÁRVORE (Renan, 24/09/2026: "chame tudo de Van, não 'Rota (Van)'").** O filtro Projeto da Árvore de Combustível listava ROTA · ROTA (VAN) · VAN. Medido pelo **Arvore Comb Inspect** (que passou a olhar a coluna `PROJ. - COD` da Dispersão, a que a Árvore lê): o DRE e a Dispersão de km escrevem `ROTA (VAN) - GRL`; a aba Km/L escreve `VAN - GRL`. Como a árvore junta as três fontes pelo rótulo, as vans viravam dois projetos — um com custo e km, outro com km/L e R$/L — em nove unidades (BLC, CBA, CGR, FLP, GRL, MCC, NFR, PLT, RON). Com "Todos" o total fechava; filtrando ROTA (VAN) o Km/L ficava "—" e filtrando VAN o custo zerava. `VAN_ALIAS` no `getNv3Prefix` da Árvore lê os dois nomes como VAN; o Check de Metas (`projArv`) pede `VAN|ROTA (VAN)` no slide da árvore, porque a tabela `fca` guarda o nome do DRE. **Não mexido**: Visão Financeira, Painel KM e FCA continuam mostrando `ROTA (VAN)`, que é o rótulo do próprio DRE.

**KM/L REMUNERADO = RÉGUA POR LITROS ESPERADOS, UMA CONTA SÓ (Renan, 24/09/2026: "Deve ler dos painéis oficiais… Só mude caso depois de eu avaliar se mudaria muito a colocação" → avaliou → "altere").** O GRL de ago/26 saía **95%** no Frota de Elite e **−7,7%** no painel Km/L com a mesma aba e o mesmo realizado (Σkm ÷ Σlitros = 3,38). Cada um fazia a sua conta: o painel tirava a média do `KM/l Rem Médio` por projeto e pesava pelo KM; o `gerot-base` (`combUm`) fazia a média simples de todas as placas. **As duas estavam erradas no mesmo sentido**: o realizado junta os projetos pelos LITROS e o remunerado os juntava pelo km ou pela placa, e ROTA (~3 km/L) com VAN (~7 km/L) punha a unidade ABAIXO dos próprios projetos — o GRL bateu o remunerado em ROTA (+1,8%) e em VAN (+15,4%) e aparecia perdendo. **Régua nova, nos quatro lugares** (painel Km/L `aggRows`, Consumo Km/L Análise `aggRows`, Árvore `kmlAgg`, `gerot-base` `loadComb`/`combUm`/`combAcum` → Frota de Elite, Gerot, Scorecard/Diagnóstico/Resumo, painel-metas, FCAs de RPM): rem de cada **projeto × mês** = média simples do `KM/l Rem Médio`; litros previstos = km ÷ esse rem; **Km/L Rem = Σkm ÷ Σprevistos**; atingimento = previstos ÷ gastos. GRL ago/26: 3,28 rem · **+2,9% / 102,9%**. É a mesma conta da fusão do MACACU (18/08) e do Km/L da Seara (11/09). O modo "Rem Modelo" do painel não mudou.
- **Avaliado ANTES de trocar** (workflow **Elite Comb Simula**, `scripts/elite-comb-simula.mjs`, roda o gerot-base real e refaz pontuação e posição mês a mês): de jan a jul 0–6 unidades trocam de posição, quase sempre 1 casa; **ago/26 6 trocam, até 3 casas, pódio Macacu · Pelotas · Piraí** (era Macacu · Nova Friburgo · Pelotas); **acumulado: Nova Friburgo passa Macacu no 1º** por décimos. A pontuação muda até 1,6 ponto (combustível pesa 10%); a colocação mexe porque o meio da tabela está empatado (2º ao 8º de ago em 0,8 ponto). Depois do deploy o controle da simulação passa a conferir a régua C (`CONTROLE=C`).
- **R$/L REMUNERADO DA VAN = O DO ROTA DA MESMA UNIDADE (Renan, 24/09/2026: "R$/L das Vans pode considerar os mesmos dos projetos rotas de suas respectivas unidades").** A aba `R$/L` do `KML_ID` não tem preço para VAN, e a Árvore mostrava R$/L rem 0,00 com Δ de 714% e Impacto Preço zerado. O `remPriceFor` dos quatro leitores do preço (Árvore, Eficiência Km/L, Consumo Km/L Análise, Preço R$/L) tenta a própria chave e, só se não achar, troca `VAN - XXX` / `ROTA (VAN) - XXX` por `ROTA - XXX` (`_vanRota`). Se um dia a aba ganhar preço de VAN, ele vale sozinho.
- **Diagnóstico por unidade**: workflow **KmL Elite vs Painel** (`scripts/kml-elite-vs-painel.mjs`, inputs `vig`/`uni`) — abre por projeto.
- **Validação**: `scripts/kml-regua-litros-teste.mjs` roda o CÓDIGO dos quatro arquivos (vm) no caso GRL: 10 ok; contra o código de antes, 8 falham. Cache do Frota de Elite `v17 → v18` (guarda pontuação calculada); o `elite-vigencia-teste` passou a ler a chave do cache do próprio painel (fixada, envelhecia a cada versão).
